...
public class MessageListener extends ListenerAdapter {
	...
	private static final HashSet<String> MasterGuilds = new HashSet<String>(
	...
	private void handleNarration(SlashCommandEvent event) {
		...
		case "lokalna": {
			...
			String logMessage = ChatUtils.DCNarrationPrefix + " " + ChatUtils.LocalPrefix + " ["
					+ event.getMember().getEffectiveName() + "]" + " {" + (user != null ? user : warp) + "} "
					+ String.valueOf(range) + " [" + narrMessage + "]";

			Location location = user != null ? this.bot.plugin.server.getPlayer(user).getLocation()
					: FileReader.getWarp(warp);

			PlayerUtils.getPlayersWithinRange(location, range).stream().forEach(p -> p.sendMessage(message));
	...
	private void handleMoneyManip(SlashCommandEvent event, String subCommand, String playerName, Boolean isBank) {
		...
		Map<String, Long> money = isBank == true ? bot.plugin.mutils.getBankMoney(uuid)
				: bot.plugin.mutils.getMoney(uuid);

		if (subCommand.equals("sprawdz")) {
			event.reply("Gracz " + playerName + " " + "Ma w " + (isBank ? "banku" : "portfelu") + ": \n" + "- **"
			...
			event.reply("Dodano **" + ile + "** " + typ + " do " + (isBank ? "banku" : "portfelu") + " gracza "
					+ playerName).queue();
			;
			return;
			...
			event.reply("Zabrano **" + ile + "** " + typ + " z " + (isBank ? "banku" : "portfelu") + " gracza "
					+ playerName).queue();
			;
			return;
		}
	...
	private boolean checkGuildPerms(Guild guild, boolean checkMaster) {
		...
		if (!MessageListener.MasterGuilds.contains(guild.getId())
