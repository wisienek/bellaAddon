...
public class MessageListener extends ListenerAdapter {
  ...
  private static final HashSet<String> AllowedGuilds = new HashSet<>(
  ...
  public void onSlashCommandInteraction(
    ...
    assert gosc != null;
    // Note: USE_SLASH_COMMANDS doesn't exist in JDA - slash commands are available
    // to all members
    // If you need permission checking, use a different permission like
    // Permission.ADMINISTRATOR
    // or implement custom permission checking logic
    ...
    switch ( event.getName() ) {
      ...
      case "link": {
        ...
        if (
          !LinkCommand.CachedKeys.containsKey( code ) || LinkCommand.CachedKeys.get( code ) == null
        ) {
  ...
  private void handleNarration(
    ...
    String user = event.getOption( "user" ) != null
        ...
        : null;
    Long range = event.getOption( "range" ) != null
        ? Objects.requireNonNull( event.getOption( "range" ) ).getAsLong()
        : 20;
    ...
    switch ( subCommand ) {
      ...
      case "lokalna": {
        ...
        String logMessage = ChatUtils.DCNarrationPrefix + " " + ChatUtils.LocalPrefix + " ["
            + Objects.requireNonNull( event.getMember() ).getEffectiveName() + "]" + " {`"
            + ( user != null ? user : warp ) + "`} <`" + range + "`> `[" + narrMessage + "]`";

        Location location = user != null ? this.bot.plugin.server.getPlayer( user ).getLocation()
      ...
      case "prywatna": {
        ...
        String logMessage = ChatUtils.DCNarrationPrefix + " " + ChatUtils.WhisperPrefix + " ["
            + Objects.requireNonNull( event.getMember() ).getEffectiveName() + " -> " + user + "]"
            + " `[" + narrMessage + "]`";
  ...
  private void handleMoneyManip(
    ...
    switch ( subCommand ) {
      case "sprawdz":
        event
            .reply( "Gracz " + playerName + " " + "Ma w " + ( isBank ? "banku" : "portfelu" )
      ...
      case "dodaj": {
        ...
        event
            .reply( "Dodano **" + ile + "** " + typ + " do " + ( isBank ? "banku" : "portfelu" )
                + " gracza " + playerName )
            .queue();
        return;
      }
      ...
      case "zabierz": {
        ...
        if ( ileMa - ile < 0 ) {
          event
              .reply( "Nie można wykonać operacji, po zabraniu będzie miał w banku mniej niż minimalna kwota!" )
              .queue();
          return;
        }
        ...
        event
            .reply( "Zabrano **" + ile + "** " + typ + " z " + ( isBank ? "banku" : "portfelu" )
                + " gracza " + playerName )
            .queue();
        return;
      }
  ...
  private void ShowPlayers(
    ...
    os.append( String
        .format( "Gracze online (%d / %d):", online.size(), bot.plugin.server.getMaxPlayers() ) );
  ...
  private void ShowOPs(
    ...
    StringBuilder os = new StringBuilder();
    os.append( String
        .format( "Ekipa online (%d / %d):", online.size(), bot.plugin.server.getMaxPlayers() ) );
  ...
  private boolean checkGuildPerms(
    ...
    return MessageListener.MasterGuilds.contains( guild.getId() )
