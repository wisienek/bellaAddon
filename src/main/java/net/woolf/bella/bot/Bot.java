...
public class Bot {
  ...
  public Bot(
    ...
    try {
      ...
      final Set<GatewayIntent> intents = new HashSet<>( Arrays
      ...
      JDABuilder builder = JDABuilder.createDefault( pwd )
          ...
          .setActivity( net.dv8tion.jda.api.entities.Activity.watching( "Online!" ) );
      ...
      plugin.logger.severe( "Error initializing Discord bot: " + e.getMessage() );
      e.printStackTrace();
    }
  }
  ...
  private void setupCommands() {
    ...
    commands.addCommands( Commands.slash( "who", "Listuje wszystkich aktywnych użytkowników" )
        .addOptions( new OptionData( OptionType.BOOLEAN, "ekipa", "czy wyświetlić też ekipę" )
            .setRequired( true ) ),

        Commands.slash( "link", "Dodaje konto serwerowe do discorda" )
            .addOption( OptionType.STRING, "code", "Kod wygenerowany w grze", true ),

        Commands.slash( "portfel", "Komendy portfelowe admina" )
            .addSubcommands( sprawdz, dodaj, zabierz ),

        Commands.slash( "bank", "Komendy bankowe admina" )
            .addSubcommands( sprawdz, dodaj, zabierz ),

        Commands.slash( "narracja", "Wysyła narrację na serwer" )
  ...
  public void sendLog(
    ...
    try {
      ...
      plugin.logger.warning( "Error sending log message: " + e.getMessage() );
    }
  }
