...
public class MoneyUtils {
	public static final Set<String> moneyTypes = new HashSet<String>(Arrays.asList("miedziak", "srebrnik", "złotnik"));
	private Main plugin;

	public MoneyUtils(Main main) {
		this.plugin = main;
	}
	...
	public Boolean deleteBank(Player player) {
		...
		for (Location bank : banks) {
			if (bank != null) {
				if (bank.getX() == nearestBank.getX() && bank.getY() == nearestBank.getY()
	...
	public List<Location> getBanks() {
		...
		for (Object item : banksList) {
			if (item != null && item instanceof String == true) {
				...
				banks.add(new Location(Bukkit.getWorld(args[0]), Double.valueOf(args[1]), Double.valueOf(args[2]),
	...
	public Boolean transferToBank(Player player, String type, Long ammount, Map<String, Long> money,
	...
	public Boolean transferFromBank(Player player, String type, Long ammount, Map<String, Long> money,
			Map<String, Long> bankMoney) {
		if (moneyTypes.contains(type) == false)
			return false;
		if (money == null)
	...
	public Boolean setConversion(String from, String to, Long conv) {
		if (conv == null || conv <= 0 || moneyTypes.contains(from) == false || moneyTypes.contains(to) == false)
			return false;
