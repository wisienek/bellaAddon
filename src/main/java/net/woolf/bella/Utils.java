...
public class Utils {
	...
	public List<Player> getNearbyPlayers(Player player, int len) {
		...
		return nearbyPlayers.stream().filter(e -> e instanceof Player && !e.getName().equals(player.getName()))
	...
	public Map<String, Object> getOTP(Player player) {
		...
		Map<String, Object> list = plugin.tps.getConfigurationSection("tps." + player.getUniqueId().toString())
				.getValues(false);
		return list;
	}
	...
	public Boolean sendOTP(Player player, String name) {
		...
		for (Player sender : sendTo)
			sender.sendMessage(
					ChatColor.DARK_GRAY + "[L]" + ChatColor.GOLD + " [Niedaleko słychać odgłos teleportacji]");
	...
	public void itemTP(Player player, NBTCompound comp) {
		...
		if (cooldownTimeOTP.containsKey(player)) {
			player.sendMessage(Main.prefixError + "Musisz odpocząć " + ChatColor.RED + cooldownTimeOTP.get(player)
