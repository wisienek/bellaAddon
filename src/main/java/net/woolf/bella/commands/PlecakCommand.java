...
public class PlecakCommand implements CommandExecutor {
  ...
  public boolean onCommand (
    ...
    if ( !sender.hasPermission(Permissions.ADMIN.toString()) ) return true;
    ...
    if ( sender instanceof Player ) {
      ...
      switch ( action ) {
        ...
        case "ustaw": {
          ...
          if ( target == null ) {
            player.sendMessage(
                Main.prefixError + "Nie znaleziono gracza " + playerName + ", sprawdz dokładną pisownię!");
            return true;
          }
          ...
          if ( target.isOnline() ) {
            ...
            player.sendMessage(
                Main.prefixInfo + String.format("Dałeś %s plecak, który trzymasz w ręce", target.getName()));
          }
  ...
  public static void addInventoryMeta (
      @NotNull ItemStack item, int rows
  ) {
  ...
  public static void setBackPackUUID (
      @NotNull ItemStack item, @NotNull String uuid
  ) {
    NBTItem nbti = new NBTItem(item);
