...
public class ListCommand implements CommandExecutor {
	...
	public boolean onCommand(
		...
		String messageToSend = StringUtils
				.synthesizeForDc( String.join( " ", Arrays.copyOfRange( args, 2, args.length ) ) );
		...
		try {
			...
			if ( playerAccounts == null ) {
				player.sendMessage( Main.prefixError
						+ "Musisz mieć podłączone konto do DC aby wysłać wiadmość!" );
				return true;
			}
			...
			if ( targetAccounts == null ) {
				player.sendMessage( Main.prefixError + "Gracz " + args[1]
						+ " nie ma podłączonych kont." );
				return true;
			}
			...
			if ( cooldownTime.containsKey( player ) ) {
				player.sendMessage( Main.prefixError + "Musisz poczekać " + ChatColor.RED
						+ cooldownTime.get( player ) + ChatColor.GRAY
						+ " sekund aby ponownie wysłać wiadomość!" );
			...
			this.plugin.logger.info( String.format( "%s sent message to %s (%s)", player
					.getName(), args[1], targetDCID[1] ) );
	...
	private void sendMessageDC(
			...
			@Nonnull String toDCID,
			@Nonnull String carrier,
			@Nonnull String message
	) {
		String synthesizedMessage = StringUtils.synthesizeForDc( message );

		this.plugin.bot.sendMessageToUser( toDCID, String
				.format( "%s Dostarczył Wiadomość od %s:\n```%s```", carrier, fromUserName, synthesizedMessage ) );

		this.plugin.bot.sendLog( String
				.format( "`%s` Wysłał Wiadomość do `%s` za pomocą `%s`:\n```%s```", fromUserName, toUserName, carrier, synthesizedMessage ), Bot.VariousLogId );
	}
