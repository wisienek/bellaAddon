...
public class ListCommand implements CommandExecutor {
  ...
  public boolean onCommand(
    ...
    try {
      ...
      this.plugin.logger.info( String
          .format( "%s sent message to %s (%s)", player.getName(), args[1], targetDCID[1] ) );
  ...
  private void sendMessageDC(
    ...
    this.plugin.bot.sendLog( String
        .format( "`%s` Wysłał Wiadomość do `%s` za pomocą `%s`:\n```%s```", fromUserName, toUserName, carrier, synthesizedMessage ), BotChannels.VariousLogId
            .toString() );
  }
