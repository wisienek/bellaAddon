		...
		if (sender instanceof Player) {
			...
			if (target == null) {
				if (player.hasPermission("portfel.admin") == false) {
					player.sendMessage(Main.prefixError + "Nie znaleziono gracza " + ChatColor.RED + to + ChatColor.GRAY
							+ " obok ciebie!");
					return true;
				}
			...
			case "pokaz": {
				...
				player.sendMessage(
						Main.prefixInfo + "Pokazano portfel dla gracza " + ChatColor.GREEN + target.getName());
			...
			case "daj": {
				...
				target.sendMessage(Main.prefixInfo + "Otrzymałeś " + ChatColor.YELLOW + ammount + " " + type + "ów"
						+ ChatColor.GRAY + " od " + player.getName());
				...
				plugin.bot.moneyLog("**" + player.getName() + "** przekazał *" + ammount + " " + type
						+ "ów* dla gracza **" + to + "**" + "\n" + player.getName() + " ma teraz: " + money.get(type)
			...
			case "ustaw": {
				...
				player.sendMessage(Main.prefixInfo + "Ustawiono stan portfela " + ChatColor.GREEN + to + ChatColor.WHITE
						+ " na " + ChatColor.YELLOW + ammount + " " + type + "ów");

				plugin.bot.moneyLog("**" + player.getName() + "** Ustawił *" + type + "i* w portfelu gracza **" + to
						+ "** na " + ammount);
			...
			case "dodaj": {
				...
				player.sendMessage(Main.prefixInfo + "Dodano " + ChatColor.YELLOW + ammount + " " + type + "ów"
						+ ChatColor.WHITE + " do konta " + ChatColor.GREEN + to + ChatColor.WHITE + ", teraz ma: "
						+ ChatColor.YELLOW + Long.valueOf(ammount) + pm.get(type));

				plugin.bot.moneyLog("**" + player.getName() + "** Dodał *" + ammount + " " + type
						+ "ów* do portfela gracza **" + to + "**" + "\n" + "Teraz ma: "
						+ String.valueOf(Long.valueOf(ammount) + pm.get(type)) + type + "ów");
			...
			case "zabierz": {
				...
				if (pm.get(type) - ammount < 0) {
					player.sendMessage(Main.prefixError + "Nie można zabrać tyle kasy, gracz będzie na minusie! "
							+ ChatColor.RED + pm.get(type) + " -> " + (pm.get(type) - ammount));
					return true;
				}
				...
				player.sendMessage(Main.prefixInfo + "Zabrano " + ChatColor.YELLOW + ammount + " " + type + "ów"
						+ ChatColor.WHITE + " z konta gracza " + ChatColor.GREEN + to + ChatColor.WHITE + ", Teraz ma: "
						+ ChatColor.YELLOW + (pm.get(type) - Long.valueOf(ammount)));

				plugin.bot.moneyLog("**" + player.getName() + "** Zabrał *" + ammount + " " + type
						+ "ów* z portfela gracza **" + to + "**" + "\n" + "Teraz ma: "
						+ String.valueOf(pm.get(type) - Long.valueOf(ammount)) + type + "ów");
