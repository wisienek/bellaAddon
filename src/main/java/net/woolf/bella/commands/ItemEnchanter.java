...
public class ItemEnchanter implements CommandExecutor {
    ...
    if ( label.equals( "zaczaruj" ) ) {
      if ( !player.hasPermission( Permissions.ENCHANTER.toString() ) ) {
        player.sendMessage( Main.prefixError + "Nie masz permissi " + ChatColor.DARK_RED
            + Permissions.ENCHANTER + ChatColor.GRAY + " aby użyć tej komendy" );
        return true;
      }
      ...
      switch ( action ) {
        case "efekt": {
          ...
          if ( enchants != null && enchants.contains( effect ) ) {
            player.sendMessage( Main.prefixError + "Przedmiot ma już dany efekt: " + ChatColor.GREEN
      ...
      if ( !player.hasPermission( Permissions.CHECK_ENCHANT.toString() ) ) {
        player.sendMessage( Main.prefixError + "Nie masz permissi " + ChatColor.DARK_RED
            + Permissions.CHECK_ENCHANT + ChatColor.GRAY + " aby użyć tej komendy" );
        return true;
      }
      ...
      if ( enchants != null )
        player.sendMessage( Main.prefixInfo + "Przedmiot jest zaczarowany na: " + ChatColor.GREEN
            + String.join( ", ", enchants ) );
      else
  ...
  public static boolean isEnchanted(
    ...
    for ( String effect : ItemEnchanter.AvailableEnchantEffects )
      if ( nbt.hasKey( effect ) && nbt.getBoolean( effect ) )
        return true;

    return false;
  }

  public static List<String> getItemEffects(
      ItemStack item
  ) {
    NBTItem nbt = new NBTItem( item );
  ...
  public static void teleportPlayerWithItem(
      Player player,
      NBTItem nbti
  ) {
    ...
    if ( Main.getInstance().teleportUtils.itemTP( player, comp ) ) {
      ...
      ChatUtils.cacheMessageForBotLog( BotChannels.VariousLogId.toString(), String
          .format( "[%s] teleportował {%d %d %d} -> {%d %d %d} (item)", player.getName(), playerLoc
              .getBlockX(), playerLoc
                  .getBlockY(), playerLoc.getBlockZ(), x.intValue(), y.intValue(), z.intValue() ) );
    }
  }
}
