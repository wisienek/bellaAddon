	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (player.hasPermission("bella.otp.use")) {
			...
			case "info": {
				...
				os.append("\n" + ChatColor.GRAY + "Max pkt tp : " + ChatColor.YELLOW + String.valueOf(len) + " / "
			...
			case "set": {
				...
				if (len >= maxpts) {
					player.sendMessage(
							Main.prefixError + "Nie możesz mieć więcej punktów tp (" + String.valueOf(maxpts) + ")!");
					return true;
				}
				...
				if (plugin.config.getBoolean("setOTP-command-delay")) {
					if (plugin.utils.cooldownTimeSetOTP.containsKey(player)) {
						...
						player.sendMessage(Main.prefixInfo + "Ustawiono TP : " + ChatColor.AQUA + otp + ChatColor.WHITE
								+ ". Zajęte tp: " + String.valueOf(len + 1) + " / " + String.valueOf(maxpts));
					}
				} else {
					setPlayerOTP(player, otp);
					player.sendMessage(Main.prefixInfo + "Ustawiono TP : " + ChatColor.AQUA + otp + ChatColor.WHITE
			...
			case "del": {
				...
				if (plugin.utils.tpsIsNull(player, otp)) {
					...
					player.sendMessage(Main.prefixInfo + "Usunięto teleport: " + ChatColor.AQUA + otp + ChatColor.WHITE
							+ ". Wolne: " + String.valueOf(len - 1) + " / " + String.valueOf(maxpts));
				}
			...
			case "seteffect": {
				...
				if (Utils.types.contains(otp) == false) {
					...
					player.sendMessage(Main.prefixError + "Twój efekt nie znajduje się na liście: " + ChatColor.YELLOW
			...
			case "tp": {
				...
				if (plugin.utils.tpsIsNull(player, otp)) {
					...
					if (distance > radius) {
						player.sendMessage(
								Main.prefixError + "Za długi dystans pomiędzy lokacją, a punktem teleportacji! ( "
										+ String.valueOf((int) distance) + " / " + String.valueOf((int) radius) + " )");
						return true;
					}
			...
			case "enchant": {
				...
				String maxu = args.length >= 3 ? args[2]
						: player.hasPermission("atp.admin") ? String.valueOf(Integer.MAX_VALUE)
				...
				player.sendMessage(Main.prefixInfo + "Enchantowano item na teleport do punktu " + ChatColor.GOLD + otp
						+ ChatColor.AQUA + " (MaxU: " + maxu + ")" + "!");
			...
			case "inspect": {
				...
				player.sendMessage(Main.prefixInfo + "Informacje o Zaczarowanym przedmiocie:\n" + ChatColor.GREEN
						+ "Enchanter: " + ChatColor.AQUA + enchanter + "\n" + ChatColor.GREEN + "Odległość: "
						+ ChatColor.AQUA + maxLen + "\n" + ChatColor.GREEN + "Odpoczynek: " + ChatColor.AQUA + enchCld
			...
			case "ws": {
				...
				if (nearbyPlayers.size() > maxp) {
					player.sendMessage(Main.prefixError + "Za dużo osób do teleportacji! ( "
							+ String.valueOf(nearbyPlayers.size()) + " / " + String.valueOf(maxp) + " )");
					return true;
				}
				...
				if (plugin.utils.tpsIsNull(player, otp)) {
					...
					if (plugin.config.getBoolean("OTP-command-delay")) {
						if (plugin.utils.cooldownTimeOTP.containsKey(player)) {
							...
							plugin.utils.setCoolDownTimeOTP(player, cld);
							player.sendMessage(Main.prefixInfo + "Teleportowano wspólnie z ( " + os.toString()
