...
public class OtpCommand implements CommandExecutor {
  ...
  public String getUsage () {
    return Main.prefixInfo + "Użycie komendy: /otp" + "\n/otp info - info o aktualnym lvl" + "\n/otp set/del/tp/ws/os"
        ...
        + " ws tp1" + "\n/otp os tp1 Zbyszek" + "\n/otp seteffect ignis" + "\n/otp enchant <punkt tp> [ilość użyć] - "
        + "ustawia " + "teleport na przedmiot z ewentualną ilością użyć" + "\n/otp inspect - robi inspekcję " +
  ...
  public boolean onCommand (
    ...
    if ( player.hasPermission(Permissions.OTP_USE.toString()) ) {
      ...
      int radius = Integer.parseInt((String) plugin.config.get("tp-level-" + levelS + "-radius"));
      int maxp = Integer.parseInt((String) plugin.config.get("tp-level-" + levelS + "-maxp"));
      int maxpts = Integer.parseInt((String) plugin.config.get("tp-level-" + levelS + "-maxpoints"));
      int setmaxuse = Integer.parseInt((String) plugin.config.get("tp-level-" + levelS + "-setmaxuse"));
      ...
      switch ( args[0] ) {
        ...
        case "set": {
          ...
          if ( plugin.config.getBoolean("setOTP-command-delay") ) {
            if ( plugin.utils.cooldownTimeSetOTP.containsKey(player) ) {
              player.sendMessage(
                  Main.prefixError + "Musisz poczekać " + ChatColor.RED + plugin.utils.cooldownTimeSetOTP.get(
              ...
              player.sendMessage(
                  Main.prefixInfo + "Ustawiono TP : " + ChatColor.AQUA + otp + ChatColor.WHITE + ". Zajęte tp: " + ( len + 1 ) + " / " + maxpts);
            }
          }
          else {
            setPlayerOTP(player, otp);
        ...
        case "del": {
          ...
          if ( plugin.utils.tpsIsNull(player, otp) ) {
            ...
            player.sendMessage(
                Main.prefixInfo + "Usunięto teleport: " + ChatColor.AQUA + otp + ChatColor.WHITE + ". Wolne: " + ( len - 1 ) + " / " + maxpts);
          }
        ...
        case "effect": {
          if ( player.hasPermission(Permissions.ATP_ADMIN.toString()) || player.hasPermission(
              Permissions.TEST.toString()) ) {
            String type = plugin.utils.getType(player);
        ...
        case "seteffect": {
          ...
          if ( !Utils.types.contains(otp) ) {
            player.sendMessage(
                Main.prefixError + "Twój efekt nie znajduje się na liście: " + ChatColor.YELLOW + String.join(", ",
                    Utils.types));
            return true;
          }
        ...
        case "tp": {
          ...
          if ( plugin.utils.tpsIsNull(player, otp) ) {
            ...
            if ( distance > radius ) {
              player.sendMessage(
                  Main.prefixError + "Za długi dystans pomiędzy lokacją, a punktem teleportacji! ( " + (int) distance + " / " + radius + " )");
              return true;
            }
        ...
        case "list": {
          ...
          for ( String key : list.keySet() ) {
            ...
            os.append("\n- ").append(key).append(" (").append((int) playerLoc.distance(loc)).append("m)");
        ...
        case "enchant": {
          if ( !player.hasPermission(Permissions.OTP_ENCHANT.toString()) ) {
            player.sendMessage(
                Main.prefixError + "Musisz posiadać umiejętność enchatnowania aby tego użyć! (bella.otp.enchant)");
            return true;
          }
        ...
        case "inspect": {
          if ( !player.hasPermission(Permissions.OTP_ENCHANT.toString()) ) {
            player.sendMessage(
                Main.prefixError + "Musisz posiadać umiejętność enchatnowania aby tego użyć! (" + Permissions.OTP_ENCHANT + ")");
            return true;
          }
          ItemStack item = player.getInventory().getItemInMainHand();
          ...
          String maxUse = comp.hasKey("maxUse") ? comp.getInteger("maxUse") == Integer.MAX_VALUE
                                                  ? "Nieskończoności"
                                                  : String.valueOf(comp.getInteger("maxUse")) : "Nieskończoności";
        ...
        case "os": {
          ...
          Player target = null;
          for ( Player p : list )
            if ( p.getName().equals(pname) ) target = p;
          ...
          if ( plugin.config.getBoolean("OTP-command-delay") ) {
            if ( plugin.utils.cooldownTimeOTP.containsKey(player) ) {
              ...
              target.sendMessage(
                  Main.prefixInfo + "Zostałeś teleportowany przez " + player.getName() + " do punktu " + otp);
        ...
        case "ws": {
          ...
          if ( nearbyPlayers.size() > maxp ) {
            player.sendMessage(
                Main.prefixError + "Za dużo osób do teleportacji! ( " + nearbyPlayers.size() + " / " + maxp + " )");
            return true;
          }
          ...
          if ( plugin.utils.tpsIsNull(player, otp) ) {
            ...
            for ( Player sending : sendTo )
              sending.sendMessage(
                  ChatColor.WHITE + "[L] " + ChatColor.YELLOW + "[Niedaleko słychać trzask teleportacji łącznej]");
            ...
            if ( plugin.config.getBoolean("OTP-command-delay") ) {
              if ( plugin.utils.cooldownTimeOTP.containsKey(player) ) {
                ...
                for ( Player target : nearbyPlayers ) {
                  ...
                  target.sendMessage(
                      Main.prefixInfo + player.getDisplayName() + " Teleportował się z tobą do punktu: " + ChatColor.YELLOW + otp);
                }
                plugin.utils.tpEffect(player, otp, null);

                plugin.utils.setCoolDownTimeOTP(player, cld, false);
                player.sendMessage(
                    Main.prefixInfo + "Teleportowano wspólnie z ( " + os + " ) do punktu " + ChatColor.YELLOW + otp);
