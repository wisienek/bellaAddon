...
public class AtpCommand implements CommandExecutor {
  ...
  public boolean onCommand (
    ...
    if ( !( sender instanceof Player ) || sender.hasPermission(Permissions.ATP_ADMIN.toString()) ) {
      ...
      switch ( args[0] ) {
        case "edit":
          ...
          if ( val.equals("infinity") ) val = String.valueOf(Integer.MAX_VALUE);
        ...
        case "seteffect": {
          if ( args.length < 3 ) sender.sendMessage(getUsage());
          ...
          if ( effect == null || effect.isEmpty() || !Utils.types.contains(effect) ) {
            sender.sendMessage(
                Main.prefixError + "Efekt nie znajduje się na liście: " + ChatColor.YELLOW + String.join(", ",
                    Utils.types));
            return true;
          }
          ...
          if ( target != null ) {
            ...
            sender.sendMessage(
                Main.prefixInfo + "Ustawiono efekt gracza " + ChatColor.GREEN + pname + ChatColor.WHITE + " na: " + ChatColor.AQUA + effect);
            return true;
          }
          break;
        }
