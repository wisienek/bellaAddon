		...
		if( sender instanceof Player ) {
			...
			switch (make) {
				...
				case "przelej" : {
					...
					player.sendMessage(
						Main.prefixInfo + "Przelano " +
						ChatColor.YELLOW + ammount + " " + type + "ów" +
						ChatColor.WHITE + " na konto gracza " +
				...
				case "wymień": case "zamień": {
					...
					plugin.logger.info( "Get: " + get + ", take: " + take + ", ammount - tax: " + (ammount-tax) + ", conversion: " + conversion );
					if( bfrom < take && mfrom < take ) {
						player.sendMessage( Main.prefixError + "Nie masz odpowiedniej sumy: " + ChatColor.YELLOW + take+ " " + from + "ów (wliczony podatek)" );
						return true;
					}
				...
				case "ustaw" : {
					if( !player.hasPermission("bank.admin") || (args.length < 4) ) { player.sendMessage( getUsage(player) ); return true; }
					Long ammount = args.length >= 3 ? Long.valueOf(args[2]) : 0l;
				...
				case "dodaj" : {
					...
					player.sendMessage(
						Main.prefixInfo + "Dodano " +
						ChatColor.YELLOW + ammount + " " + type + "ów" +
						ChatColor.WHITE + " do konta gracza " +
						ChatColor.GREEN + args[3] +
						ChatColor.WHITE + ", teraz ma: " +
				...
				case "stwórz": {
					if( !player.hasPermission("bank.admin") ) { player.sendMessage( getUsage(player) ); return true; }
