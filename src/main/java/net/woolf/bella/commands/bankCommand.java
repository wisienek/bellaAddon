...
public class bankCommand implements CommandExecutor {
	...
	public String getUsage(Player player) {
		...
		return Main.prefixInfo + "Użycie komendy: /bank" + "\n/bank - pokazuje stan banku"
				...
				+ "\n/bank wymień <z czego> <na co> <ile chcesz wymienić> - wymienia jedną walutę na drugą" + add;
	}
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if (sender instanceof Player) {
			...
			if (!player.hasPermission("bank.admin") && !plugin.mutils.isNearBank(player.getLocation())) {
			...
			if (args.length == 0) {
				player.sendMessage(Main.prefixInfo + "Twoje pieniążki w banku: \n" + ChatColor.WHITE + "Miedziaki : "
						...
						+ String.valueOf(bankMoney.get("srebrnik")) + "\n" + ChatColor.YELLOW + "Złotniki : "
			...
			case "sprawdz": {
				...
				player.sendMessage(Main.prefixInfo + "Stan banku dla gracza " + type + ": \n" + ChatColor.WHITE
						+ "Miedziaki : " + String.valueOf(targetBankMoney.get("miedziak")) + "\n" + ChatColor.GRAY
						+ "Srebrniki : " + String.valueOf(targetBankMoney.get("srebrnik")) + "\n" + ChatColor.YELLOW
						+ "Złotniki : " + String.valueOf(targetBankMoney.get("złotnik")));
				return true;
			}
			case "wpłać": {
				...
				if (rest < 0) {
					player.sendMessage(Main.prefixError + "Nie możesz wpłacić " + ChatColor.YELLOW + ammount
							+ ChatColor.GRAY + " bo twój balans portfela będzie na minusie: " + ChatColor.RED + rest);
					return true;
				}
				...
				money.put(type, rest);
				player.sendMessage(Main.prefixInfo + "Wpłacono " + ChatColor.YELLOW + ammount + " " + type + "ów"
						+ ChatColor.WHITE + ", Aktualny stan konta: " + ChatColor.YELLOW + bankMoney.get(type) + " "
						+ type + "ów");
			...
			case "wypłać": {
				...
				if (rest < 0) {
					player.sendMessage(Main.prefixError + "Nie możesz wypłacić " + ChatColor.YELLOW + ammount
							+ ChatColor.GRAY + " bo twój balans banku będzie na minusie: " + ChatColor.RED + rest);
					return true;
				}
				...
				player.sendMessage(Main.prefixInfo + "Wypłacono " + ChatColor.YELLOW + ammount + " " + type + "ów"
						+ ChatColor.WHITE + ", Aktualny stan konta: " + ChatColor.YELLOW + bankMoney.get(type) + " "
						+ type + "ów");
			...
			case "przelej": {
				...
				if (!done) {
					player.sendMessage(
							Main.prefixError + "Błąd podczas dodawania, sprawdz pisownie i czy gracz jest online.");
					return true;
				}

				player.sendMessage(Main.prefixInfo + "Przelano " + ChatColor.YELLOW + ammount + " " + type + "ów"
						+ ChatColor.WHITE + " na konto gracza " + ChatColor.GREEN + args[3]);

				plugin.bot.moneyLog("**" + player.getName() + "** Przelał *" + ammount + " " + type
						+ "ów* na konto gracza **" + args[3] + "**!");
			...
			case "zamień": {
				...
				plugin.logger.info("Get: " + get + ", take: " + take + ", ammount - tax: " + (ammount - tax)
						+ ", conversion: " + conversion);
				if (bfrom < take && mfrom < take) {
					player.sendMessage(Main.prefixError + "Nie masz odpowiedniej sumy: " + ChatColor.YELLOW + take + " "
							+ from + "ów (wliczony podatek)");
					return true;
				}
				...
				if (!taken) {
					...
					Boolean check = plugin.mutils.setBankMoney(player.getUniqueId().toString(), to,
							bankMoney.get(to) + get);
					if (!check) {
						player.sendMessage(Main.prefixError + "Nie udało się dodać kasy do banku!");
						return true;
					}

					player.sendMessage(Main.prefixInfo + "Prawidłowo wymieniono " + ChatColor.YELLOW + take + " " + from
							+ "ów" + ChatColor.WHITE + " na " + ChatColor.YELLOW + get + " " + to + "ów");
					plugin.bot.moneyLog("**" + player.getName() + "** Wymienił *" + take + " " + from + "ów* na *" + get
							+ " " + to + "ów*!");
			...
			case "ustaw": {
				...
				if (!done) {
					player.sendMessage(
							Main.prefixError + "Błąd podczas setowania, sprawdz pisownie i czy gracz jest online.");
					return true;
				}

				player.sendMessage(Main.prefixInfo + "Ustawiono stan konta gracza " + ChatColor.GREEN + args[3]
						+ ChatColor.WHITE + " na " + ChatColor.YELLOW + ammount + " " + type + "ów");
				plugin.bot.moneyLog("**" + player.getName() + "** Ustawił *" + type + "i* gracza **" + args[3]
						+ "** na " + ammount);
			...
			case "dodaj": {
				...
				player.sendMessage(Main.prefixInfo + "Dodano " + ChatColor.YELLOW + ammount + " " + type + "ów"
						+ ChatColor.WHITE + " do konta gracza " + ChatColor.GREEN + args[3] + ChatColor.WHITE
						+ ", teraz ma: " + ChatColor.YELLOW + (Long.valueOf(bm.get(type)) + ammount));

				plugin.bot.moneyLog("**" + player.getName() + "** Dodał *" + ammount + " " + type
						+ "ów* do banku gracza **" + args[3] + "**!");
			...
			case "zabierz": {
				...
				if (bm.get(type) - ammount < 0) {
					player.sendMessage(Main.prefixError + "Nie można zabrać tyle kasy, gracz będzie na minusie! "
							+ ChatColor.RED + bm.get(type) + " -> " + (bm.get(type) - ammount));
					return true;
				}
				...
				if (!done) {
					player.sendMessage(
							Main.prefixError + "Błąd podczas zabierania, sprawdz pisownie i czy gracz jest online.");
					return true;
				}

				player.sendMessage(Main.prefixInfo + "Zabrano " + ChatColor.YELLOW + ammount + " " + type + "ów"
						+ ChatColor.WHITE + " z konta gracza " + ChatColor.GREEN + args[3] + ChatColor.WHITE
						+ ", teraz ma: " + ChatColor.YELLOW + (bm.get(type) - ammount));
				plugin.bot.moneyLog("**" + player.getName() + "** Zabrał *" + ammount + " " + type
						+ "ów* z banku gracza **" + args[3] + "**!");
			...
			case "stwórz": {
				...
				if (type.equals("miejsce")) {
					...
					plugin.bot.moneyLog("**" + player.getName() + "** Ustawił nowy bank w lokalizacji (" + loc.getX()
							+ " " + loc.getY() + " " + loc.getZ() + ")");
				}
			...
			case "usuń": {
				...
				plugin.bot.moneyLog("**" + player.getName() + "** Usunął bank w lokalizacji (" + loc.getX() + " "
						+ loc.getY() + " " + loc.getZ() + ")");
			...
			case "konwersje": {
				...
				if (!check) {
					player.sendMessage(Main.prefixError + "Nie udało się ustawić konwersji: " + from + " * " + conv
							+ " = 1 * " + to);
					return true;
				}

				player.sendMessage(Main.prefixInfo + "Ustawiono konwersję: " + from + " * " + conv + " = 1 * " + to);
				plugin.bot.moneyLog("**" + player.getName() + "** Ustawił konwersję: `" + from + " -> " + to + "` **"
						+ conv + " : 1**");
			...
			case "list": {
				...
				for (Location bank : banks)
					os.append(ChatColor.GREEN + "\nX: " + ChatColor.RED + bank.getBlockX() + ChatColor.GREEN + ", Y: "
							+ ChatColor.RED + bank.getBlockY() + ChatColor.GREEN + ", Z: " + ChatColor.RED
							+ bank.getBlockZ());
