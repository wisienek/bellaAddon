    ...
    if ( sender instanceof Player ) {
      ...
      if ( target == null ) {
        if ( !player.hasPermission( Permissions.PORTFEL_ADMIN.toString() ) ) {
          player.sendMessage( Main.prefixError + "Nie znaleziono gracza " + ChatColor.RED + to
              + ChatColor.GRAY + " obok ciebie!" );
          return true;
        }
        ...
        if ( target == null ) {
          player.sendMessage( Main.prefixError + "Nie znaleziono gracza " + ChatColor.RED + to
              + ChatColor.GRAY + ", aby pokazać mu " + "portfel!" );
          return true;
        }
      }
      ...
      switch ( make ) {
        ...
        case "daj": {
          ...
          ChatUtils.cacheMessageForBotLog( BotChannels.MoneyLogId.toString(), String
              .format( "**%s** przekazał *%s %sów* dla gracza **%s**\n%s ma teraz: %s %sów\n**%s** ma teraz: %s %sów", player
                  .getName(), ammount, type, to, player.getName(), money
                      .get( type ), type, target.getName(), toMoney.get( type ), type ) );
        ...
        case "ustaw": {
          ...
          player.sendMessage( Main.prefixInfo + "Ustawiono stan portfela " + ChatColor.GREEN + to
              + ChatColor.WHITE + " na " + ChatColor.YELLOW + ammount + " " + type + "ów" );

          ChatUtils.cacheMessageForBotLog( BotChannels.MoneyLogId.toString(), String
              .format( "**%s** Ustawił *%si* w portfelu gracza **%s** na %s", player
                  .getName(), type, to, ammount ) );
        ...
        case "dodaj": {
          ...
          player.sendMessage( Main.prefixInfo + "Dodano " + ChatColor.YELLOW + ammount + " " + type
              + "ów" + ChatColor.WHITE + " do " + "konta " + ChatColor.GREEN + to + ChatColor.WHITE
              + ", teraz ma: " + ChatColor.YELLOW + (long) ammount + " " + pm.get( type ) );

          ChatUtils.cacheMessageForBotLog( BotChannels.MoneyLogId.toString(), String
              .format( "**%s** Dodał *%s %sów* do portfela **%s**\nTeraz ma: %s %sów", player
                  .getName(), ammount, type, to, pm.get( type ) + (long) ammount, type ) );
        ...
        case "zabierz": {
          ...
          if ( pm.get( type ) - ammount < 0 ) {
            player.sendMessage( Main.prefixError
                ...
                + pm.get( type ) + " -> " + ( pm.get( type ) - ammount ) );
            return true;
          }
          ...
          player.sendMessage( Main.prefixInfo + "Zabrano " + ChatColor.YELLOW + ammount + " " + type
              + "ów" + ChatColor.WHITE + " z " + "konta gracza " + ChatColor.GREEN + to
              + ChatColor.WHITE + ", Teraz ma: " + ChatColor.YELLOW
              + ( pm.get( type ) - (long) ammount ) );

          ChatUtils.cacheMessageForBotLog( BotChannels.MoneyLogId.toString(), String
              .format( "**%s** Zabrał *%s %sów* z portfela **%s**\nTeraz ma: %s %sów", player
                  .getName(), ammount, type, to, pm.get( type ) - (long) ammount, type ) );
